...
/** Offline fallbacks from curated datasets when API server is unreachable */
export const mockCareerViews = (): CareerView[] =>
  CAREERS_DATA.map(c => ({
    id: c.id,
    title: c.title,
    category: c.category,
    categoryKey: plainCategory(c.category),
    shortOverview: c.shortOverview,
    description: c.description,
    whatYouDo: c.whatYouDo,
    icon: c.icon,
    relatedMajorsText: c.relatedMajorsText,
    keySkills: c.keySkills,
    educationRequired: c.educationRequired,
    bestFitPersonality: c.bestFitPersonality,
    jobMarketDemand: c.jobMarketDemand,
    source: null,
    sourceUrl: null,
  }));

export const mockMajorViews = (): MajorView[] =>
  MAJORS_DATA.map(m => ({
    id: m.id,
    name: m.name,
    category: m.category,
    categoryKey: plainCategory(m.category),
    ...
    personalityFit: [],
    jobMarketDemand: m.jobMarketDemand,
    relatedCareersText: m.careerPathways ? m.careerPathways.map(p => p.title) : [],
    universitiesText: m.offerUniversities ? m.offerUniversities.map(u => u.name) : [],
    relatedScholarshipsText: m.relatedOpportunities ? m.relatedOpportunities.map(o => o.title) : [],
    source: m.source ?? null,
    sourceUrl: m.sourceUrl ?? null,
    extendedDescription: m.extendedDescription ?? null,
    whatYouLearn: m.whatYouLearn ?? [],
    skillsDeveloped: m.skillsDeveloped ?? [],
    careerOpportunities: m.careerOpportunities ?? "",
  }));

export const mockUniversityViews = (): UniversityView[] =>
  UNIVERSITIES_DATA.map((u, idx) => ({
    id: u.id,
    apiId: idx + 1,
    name: u.name,
    ...
    scholarshipsList: u.scholarshipsList ?? [],
  }));
