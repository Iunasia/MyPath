...
import { getCareersTranslated } from "@/app/data/careers";
...
export default async function CareerDetailPage({ params }: PageProps) {
  ...
  const translatedCareers = await getCareersTranslated(locale);
  const fallbackItem = translatedCareers.find(
  ...
  const apiView = row ? toCareerView(row) : null;

  const career: CareerView = apiView
    ? {
        ...apiView,
        title: fallbackItem?.title ?? apiView.title,
        category: fallbackItem?.category ?? apiView.category,
        shortOverview: fallbackItem?.shortOverview ?? apiView.shortOverview,
        description: fallbackItem?.description ?? apiView.description,
        whatYouDo: fallbackItem?.whatYouDo ?? apiView.whatYouDo,
        relatedMajorsText: fallbackItem?.relatedMajorsText ?? apiView.relatedMajorsText,
        keySkills: fallbackItem?.keySkills ?? apiView.keySkills,
        educationRequired: fallbackItem?.educationRequired ?? apiView.educationRequired,
        bestFitPersonality: fallbackItem?.bestFitPersonality ?? apiView.bestFitPersonality,
        jobMarketDemand: fallbackItem?.jobMarketDemand ?? apiView.jobMarketDemand,
      }
    : {
        id: fallbackItem!.id,
  ...
  return (
    <div className="min-h-screen bg-powder text-blue-ink flex flex-col">
      <div className="w-full px-[25px] py-6 sm:px-10 lg:px-[80px] flex flex-col">
        {/* ── Header ────────────────────────────────────────── */}
        <BackLink href="/careers" label={t("allCareers")} className="mb-6" />
        ...
        <main className="w-full pb-16 flex flex-col gap-10">
          ...
          <section className="w-full">
            <div className="flex flex-wrap items-center gap-2 mb-3">
              ...
              <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-emerald-100 text-emerald-800 text-xs font-extrabold uppercase tracking-wider border border-emerald-200">
                ...
                {t("demand", { value: career.jobMarketDemand })}
              </span>
            </div>
          ...
          <section className="bg-white rounded-3xl p-6 sm:p-8 lg:p-10 border border-sky/15 bubble-shadow-sm w-full rounded-br-[86px]">
            ...
            <p className="text-xs sm:text-sm text-gray-soft mb-8 font-medium">
              {t("majorsThatLeadHereDesc", { title: career.title })}
            </p>
          ...
          <section className="bg-white rounded-3xl p-6 sm:p-8 lg:p-10 border border-sky/15 bubble-shadow-sm w-full">
            ...
            <p className="text-xs sm:text-sm text-gray-soft mb-6 font-medium">
              {t("keySkillsDevelopedDesc", { title: career.title })}
            </p>
          ...
          <div className="rounded-3xl bg-momo p-6 sm:p-8 border border-momo w-full">
            ...
            <div className="mt-4 pt-3 border-t border-blue-ink/10 flex justify-end">
              ...
              >
                {t("browseAllCareers")} <ArrowRight className="w-3.5 h-3.5" />
              </Link>
            </div>
          </div>
        </main>
