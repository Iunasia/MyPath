...
import { getMajorsTranslated } from "@/app/data/majors";
...
export default async function MajorDetailPage({ params }: PageProps) {
  ...
  const t = await getTranslations("majorDetail");
  ...
  const [translatedMajors, translatedCareers] = await Promise.all([
    getMajorsTranslated(locale),
    getCareersTranslated(locale),
  ]);

  const fallbackMajor = translatedMajors.find(
  ...
  const major: MajorView = apiView
    ? {
        ...apiView,
        name: fallbackMajor?.name ?? apiView.name,
        category: fallbackMajor?.category ?? apiView.category,
        description: fallbackMajor?.description ?? apiView.description,
        extendedDescription:
          fallbackMajor?.extendedDescription ?? apiView.extendedDescription,
  ...
  return (
    <div className="min-h-screen bg-powder text-blue-ink flex flex-col">
      ...
      <div className="w-full px-[25px] py-6 sm:px-10 lg:px-[80px] flex flex-col">
        ...
        <BackLink href="/majors" label={t("majors")} className="mb-6" />
                ...
                {hasDemand && (
                  <div className="shrink-0">
                    <span className="inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-sitomo text-xs font-bold text-sky-deep border border-sky/20 shadow-2xs">
                      {t("demand", { value: major.jobMarketDemand })}
                    </span>
                  </div>
                )}
              </div>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-soft font-medium">
                        {opp.provider} · {tCommon("deadline")}: {opp.deadline}
                      </p>
                    </div>
                  </div>
                ))}
