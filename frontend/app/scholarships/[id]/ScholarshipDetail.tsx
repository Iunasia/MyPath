...
/** Data comes from the server page (page.tsx), which also handles 404s. */
export default function ScholarshipDetail({
  ...
  // The detail endpoint returns infoCheck beside the row, not inside it (the
  // list endpoint nests it), so merge before adapting.
  const scholarship = useMemo(
    () => toScholarshipView({ ...detail.scholarship, infoCheck: detail.infoCheck }),
    [detail]
  );
  // "Similar scholarships" is picked from real data, not the static file.
